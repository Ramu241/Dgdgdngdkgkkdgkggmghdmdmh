...
async function startServer() {
  ...
  // Proxy endpoint for WinGo 30S live official history API
  app.get("/api/wingo-30s", async (req, res) => {
    try {
      const response = await fetch(
        "https://draw.ar-lottery01.com/WinGo/WinGo_30S/GetHistoryIssuePage.json?pageNo=1&pageSize=25",
        {
          headers: {
      ...
      console.error("WinGo 30S API Fetch Error:", err.message);
